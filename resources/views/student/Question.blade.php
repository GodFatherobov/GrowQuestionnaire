...
<h1>你會如何回應 ?(How would you respond?)</h1>
<h2 style="color:green">{{$question->id}}. 情境(SITUATION)</h2>
    <h3 style="color: #38c172">{{$question->Question}}</h3>
@if($question->id == 1)
    <h3>A team member is not responding lately to your friendly conversation and obvious concern for her welfare. She seems frustrated and is not finishing tasks on time. </h3>
    @elseif($question->id == 2)
    <h3>A new team member is beginning to learn, at a steady pace, the task you have assigned and his<br> performance is improving. He is prematurely asking for more challenging tasks.</h3>
    @elseif($question->id == 3)
    <h3>A team member is concerned about a recently assigned routine project. On projects like this, you have<br> normally left him alone. The team member's past performance and interpersonal relations have been good. </h3>
@elseif($question->id == 4)
    <h3>A team member has proposed a change that she wants to implement. She is enthusiastic and has a fine<br> record of accomplishment in this area. You view her suggestion as extremely valuable. </h3>
@elseif($question->id == 5)
    <h3>A team member's performance has been dropping during the last few months. She has been unconcerned<br> with meeting objectives. She continually needs coaching in ensure her tasks are done on time. </h3>
@elseif($question->id == 6)
    <h3>Based on feedback from a previous supervisor, you have been closely supervising and directing a team<br> member on a critical task. She is responding favorably and you would now like to help her become more<br> efficient. </h3>
@elseif($question->id == 7)
    <h3>You are considering changing a work procedure. You assign the task to a team member who knows the<br> procedure well. He expresses concern about his workload and the things that could go wrong. </h3>
@elseif($question->id == 8)
    <h3>A team member's performance and peer relationships are strong. You feel somewhat concerned about<br> letting her independently take on an important project. </h3>
@elseif($question->id == 9)
    <h3>You have been appointed to head a task force. One team member in particular presents a number of<br> challenges. He appears to be unclear on the goals of the task force and his attendance at scheduled<br> meetings has been sporadic. From what you have observed, he treats the meetings he does attend as social<br> gatherings. His behavior is affecting the other team members. </h3>
...
@elseif($question->id == 11)
    <h3>You have assigned task to a team member who has completed it before at an acceptable level. You have<br> allowed her to work fairly independently in the past, but she now wants your input and involvement.</h3>
@elseif($question->id == 12)
    <h3>In a recent meeting, a team member passionately stated his point of view. In this meeting, his peers<br> expressed disagreement with this team member's viewpoint. This particular team member has a remarkable<br> record of accomplishment. The team, as a whole, has worked in harmony for the past year and all are well<br> qualified for the task. </h3>
@endif
    <form action="{{ route('student.StoreAnswer',['Sid' => $Sid,'Qid'=>$Qid]) }}" enctype="multipart/form-data" method="post">
        @csrf
<div>
    <h3 style="color: blue">行動選項 – 你可能會 ...<div style="color: black"> ALTERNATIVE ACTIONS – You would...</div></h3>
        <div style="font-size: 20px">
    <div class="row">
    <input type="radio" name="answer" id="A" value="A" />
    <label for="A" style="color:royalblue">(A){{$question->Option1}}  @if($question->id == 1)
            <br><div style="padding-left: 30px;color: black">Help her learn the task step by step and let her know the consequences of missed deadlines.</div>
        @elseif($question->id == 2)
            <br><div style="padding-left: 30px;color: black">Praise his improvement, explain next steps in mastering his current tasks and explain how they connect to the bigger picture. </div>
        @elseif($question->id == 3)
...
<div>
    ...
    <label for="B" style="color:royalblue">(B) {{$question->Option2}} @if($question->id == 1)
            <br><div style="padding-left: 30px;color: black">Ask her how she plans to get back on track, and encourage her to do so.</div>
        @elseif($question->id == 2)
        ...
        @elseif($question->id == 8)
            <br><div style="padding-left: 30px;color: black">Ask for her input as you makes necessary decisions.</div>
        @elseif($question->id == 9)
...
<div>
    ...
    <label for="C" style="color:royalblue">(C) {{$question->Option3}} @if($question->id == 1)
            <br><div style="padding-left: 30px;color: black">Discuss the issue with her and then set goals.</div>
        @elseif($question->id == 2)
            <br><div style="padding-left: 30px;color: black">Provide encouragement on how quickly he is learning, and ask his opinions on the type of new assignments he would like. </div>
        @elseif($question->id == 3)
        ...
        @elseif($question->id == 7)
            <br><div style="padding-left: 30px;color: black">Listen to his concerns and explain to him why the changed procedure would work while maintaining control of the implementation.</div>
        @elseif($question->id == 8)
        ...
        @elseif($question->id == 11)
            <br><div style="padding-left: 30px;color: black">Discuss how she can build on her past performance and then provide her with specific directions for carrying out the task.</div>
        @elseif($question->id == 12)
...
<div>
    ...
    <label for="D" style="color:royalblue">(D) {{$question->Option4}} @if($question->id == 1)
            <br><div style="padding-left: 30px;color: black">Monitor performance and let her know support is available if she needs it. </div>
        @elseif($question->id == 2)
